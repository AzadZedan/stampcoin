...
describe("market module", () => {
  ...
  describe("addMarketItem", () => {
    test("adds an item with required fields", () => {
      const item = marketModule.addMarketItem("seller1", { name: "Blue Penny", price: 100 });
    ...
    test("defaults missing optional fields", () => {
      const item = marketModule.addMarketItem("seller1", { name: "Test Stamp" });
  ...
  describe("getAllMarketItems", () => {
    beforeEach(() => {
      marketModule.addMarketItem("s1", { name: "Stamp A", type: "stamp", price: 50 });
      marketModule.addMarketItem("s2", { name: "Coin B", type: "coin", price: 200 });
    });
  ...
  describe("getMarketItem", () => {
    ...
    test("throws for unknown id", () => {
      expect(() => marketModule.getMarketItem("nonexistent-id")).toThrow("Market item not found");
    });
  });
  ...
  describe("updateMarketItem", () => {
    test("updates allowed fields", () => {
      const item = marketModule.addMarketItem("s1", { name: "Old Name", price: 10 });
      const updated = marketModule.updateMarketItem(item.id, { price: 99, description: "Updated" });
      expect(updated.price).toBe(99);
      expect(updated.description).toBe("Updated");
    });

    test("throws for unknown id", () => {
      expect(() => marketModule.updateMarketItem("bad-id", { price: 1 })).toThrow("Market item not found");
    });
  });
  ...
  describe("purchaseMarketItem", () => {
    test("marks item as sold and records transaction", () => {
      const item = marketModule.addMarketItem("seller1", { name: "Stamp", price: 50 });
      ...
      expect(() => marketModule.purchaseMarketItem(item.id, "buyer2")).toThrow("not available");
    });

    test("throws when buyer is the seller", () => {
      const item = marketModule.addMarketItem("user1", { name: "Stamp", price: 10 });
      expect(() => marketModule.purchaseMarketItem(item.id, "user1")).toThrow("Cannot purchase your own item");
    });
  });
  ...
  describe("removeMarketItem", () => {
    ...
    test("throws when called by non-seller", () => {
      const item = marketModule.addMarketItem("seller1", { name: "Stamp" });
      expect(() => marketModule.removeMarketItem(item.id, "hacker")).toThrow("Only the seller");
    });

    test("throws for unknown item", () => {
      expect(() => marketModule.removeMarketItem("bad-id", "s1")).toThrow("Market item not found");
    });
  });
  ...
  describe("getMarketTransactions", () => {
    test("returns all transactions without filter", () => {
      const item = marketModule.addMarketItem("seller1", { name: "Stamp", price: 10 });
    ...
    test("filters by buyerId", () => {
      const item1 = marketModule.addMarketItem("seller1", { name: "Stamp 1", price: 10 });
      const item2 = marketModule.addMarketItem("seller1", { name: "Stamp 2", price: 20 });
